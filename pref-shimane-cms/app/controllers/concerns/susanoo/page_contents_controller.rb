# -*- coding: utf-8 -*-
module Concerns::Susanoo::PageContentsController
  ...
  included do
    ...
    before_action :set_page_content,
      only: %i(show edit update
        destroy cancel_request destroy_public_term
        edit_private_page_status update_private_page_status
        edit_public_page_status update_public_page_status)
    before_action :set_creation_params, only: %i(new create edit update)
    before_action :page_permission_required, only: %i(new edit)
    before_action :page_status_check, only: %i(new create edit update)
    before_action :private_page_status_check,
      only: %i(edit_private_page_status update_private_page_status)

    # GET /susanoo/page_contents
    def index
      @page_contents = PageContent.all
    end

    # GET /susanoo/page_contents/1
    def show
    end

    # GET /susanoo/page_contents/new
    def new
    ...
    #
    # GET /susanoo/page_contents/1/edit
    #
    def edit
      @page.lock!(request.session_options[:id], current_user)
      if params[:copy_id].present?
        copy_page_content = PageContent.find(params[:copy_id])
        if copy_page_content.page.has_permission?(current_user)
          @page_content.copy_from!(copy_page_content)
        end
      end
    ...
    #
    # GET /susanoo/page_contents/cancel
    #=== ページ編集キャンセル
    #
    def cancel
      ...
      if params[:mode] == "new_page"
        @page.destroy
        redirect_to main_app.susanoo_pages_path
      else
        @page.unlock!
        redirect_to(main_app.susanoo_page_path(@page))
      end
    end

    # PATCH/PUT /susanoo/page_contents/1
    def update
    ...
    # DELETE /susanoo/page_contents/1
    def destroy
    ...
    #
    # GET /susanoo/page_content/1/cancel_request
    #=== 公開停止依頼
    #
    def cancel_request
      begin
        ...
        redirect_to main_app.susanoo_page_path(@page), flash: {error: t(".success")}
      rescue => e
        logger.error("#{e.message}\n#{e.backtrace.join("\n")}")
        redirect_to main_app.susanoo_page_path(@page), flash: {error: t(".failure")}
      end
    end
    ...
    def destroy_public_term
      ...
      return redirect_to(main_app.susanoo_page_path(@page), notice: t(".success"))
    end
    ...
    def preview
      begin
        ...
        doc.css('html link[@rel="stylesheet"]').remove
        doc.css('html *[@style]').remove_attr('style')
        @preview_html = doc.to_xml
      rescue => e
        @fatal_message = t('.failure')
        logger.error(%Q!#{$!} : #{$@.join("\n")}!)
      end
    ...
    private
      ...
      #=== 未公開ページの状態をチェック
      #
      def private_page_status_check(page_content = @page_content)
        if !page_content.page_status_editable?
          return redirect_to main_app.susanoo_page_path(page_content.page),
