...
module BlogManagement
  module CalendarHelper
    ...
    VERSION = '0.2.2'
    JP_DAYNAMES = ['日', '月', '火', '水', '木', '金', '土']
    ABBR_JP_DAYNAMES = ['日', '月', '火', '水', '木', '金', '土']
  ...
  #    cal << %(<th colspan="#{colspan}" class="#{options[:month_name_class]}">#{Date::MONTHNAMES[options[:month]]}</th>)
      cal << %(<th colspan="#{colspan}" class="#{options[:month_name_class]}">#{options[:year]}年#{options[:month]}月</th>)
  ...
  #        cal << %(">#{d.day}<span class="hidden"> #{Date::MONTHNAMES[d.month]}</span></td>)
          cal << %(">#{d.day}<span class="hidden"> #{d.month}月</span></td>)
        else
          cal << %(">#{d.day}</td>)
        end
  ...
  #        cal << %(">#{d.day}<span class='hidden'> #{Date::MONTHNAMES[d.mon]}</span></td>)
          cal << %(">#{d.day}<span class='hidden'> #{d.mon}月</span></td>)
        else
          cal << %(">#{d.day}</td>)
        end
