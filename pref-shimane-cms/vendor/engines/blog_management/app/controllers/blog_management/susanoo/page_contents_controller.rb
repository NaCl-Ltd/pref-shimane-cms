...
module BlogManagement
  class Susanoo::PageContentsController < BlogManagement::ApplicationController
    ...
    before_action :set_creation_params, only: %i(new create edit udpate)
    before_action :page_permission_required, only: %i(edit update)
    before_action :page_status_check, only: %i(new create edit update)

    # GET /blog_management/susanoo/page_contents
    def index
      @page_contents = ::PageContent.all
    end

    # GET /blog_management/susanoo/page_contents/1
    def show
    ...
    # GET /blog_management/susanoo/page_contents/new
    def new
    ...
    #
    # GET /blog_management/susanoo/page_contents/1/edit
    #
    def edit
      @page.lock!(request.session_options[:id], current_user)
    ...
    # POST /blog_management/susanoo/page_contents
    def create
    ...
    # GET /blog_management/susanoo/page_contents/cancel
    def cancel
      @page = ::Page.find(params[:page_id])
      if params[:mode] == "new_page" &&
        @page.destroy
        redirect_to susanoo_blogs_path
      else
        @page.unlock!
    ...
    # PATCH/PUT /blog_management/susanoo/page_contents/1
    def update
    ...
    # DELETE /blog_management/susanoo/page_contents/1
    def destroy
    ...
    def update_private_page_status
      if params[:cancel].blank?
        if @page_content.update_as_private(current_user,
            ...
            params[:public_term]) &&
          @page_content.update_as_private_for_blog(
            page_content_params(:update_private_page_status_for_blog))
          redirect_to susanoo_page_path(@page),
    ...
    def update_public_page_status
      if params[:cancel].blank?
        if @page_content.update_as_public(current_user,
           ...
           @page_content.update_as_public_for_blog(
            page_content_params(:update_public_page_status_for_blog))
    ...
    def page_content_params(method)
      case method
      when :create
        params[:page_content].permit(:content, :page_id, :mobile)
      when :update
      ...
      when :update_private_page_status_for_blog, :update_public_page_status_for_blog
        params[:page_content].permit(:admission, :last_modified, :begin_date)
      end
    end
